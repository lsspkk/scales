class MusicScale {
  ...
  getScale() {
    ...
    // Extract base mode name (handle "ionian (Duuri)" -> "ionian", "aeolian (Molli)" -> "aeolian")
    const baseModeName = this.currentMode.split(' ')[0].toLowerCase()
    const intervals = this.modes[baseModeName] || this.modes.ionian
