...
- in practice screen auto-introduce variations after some time, or buttons to roll or activate/deactivate variation
- 2 or 3 position shifts in the practice routine
...
- Practice screen (Soittohetki), that can be opened from practice screen (Harjoittelunäyttö)
  timer, and some nice animations that kids like to indicate time
  the animations must be procedurally generated CSS animation, that does not require assets nor take much CPU
- Simple Drone/chord loop background for Soittohetki (one pitch per scale root)
